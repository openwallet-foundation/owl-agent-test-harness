...
```
 Running locally and not in a build pipeline/continuous integration system, you will need to install the allure framework in order to generate and display the html report. 
```
$ pip install allure-behave
```
To generate the html report and start an allure report server. 
```
