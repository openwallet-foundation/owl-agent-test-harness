...
import { TransportService } from "@aries-framework/core/build/agent/TransportService";
  ...
  public async start(agent: Agent) {
    ...
    this.app.post("/", async (req, res) => {
      ...
      try {
        ...
        config.logger.error(
          `Error processing inbound message: ${error.message}`,
          error
        );
  ...
  public async send(outboundMessage: OutboundPackage): Promise<void> {
    if (this.res.headersSent) {
      throw new AriesFrameworkError(
        `${this.type} transport session has been closed.`
      );
    }
