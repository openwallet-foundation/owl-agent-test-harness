...
pub async fn initialize() -> AgentConfig {
    ...
    let pool_config = serde_json::to_string(
    ...
    create_wallet_with_master_secret(&wallet_config)
        .await
        .expect("Failed to create wallet");
    let wallet_handle = open_wallet(&wallet_config)
        .await
        .expect("Failed to open wallet");
    create_pool_ledger_config("aries-vcx-pool", &genesis_path)
        .await
        .unwrap();
    let pool_handle = open_pool_ledger("aries-vcx-pool", Some(&pool_config))
        .await
        .unwrap();

    let issuer_config = wallet_configure_issuer(wallet_handle, &get_trustee_seed().await)
        .await
        .expect("Failed to configure the issuer wallet");
    init_issuer_config(&issuer_config).expect("Failed to init issuer config");
    let mut agency_client = AgencyClient::new();
    let agency_client_config =
        provision_cloud_agent(&mut agency_client, wallet_handle, &agency_config)
            .await
            .expect("Failed to provision the cloud agent");

    debug!("Initialization finished");
    AgentConfig {
        did: issuer_config.institution_did,
        wallet_handle,
        pool_handle,
        agency_client_config,
    }
}
