...
async fn create_and_publish_schema(
    ...
    let (schema_id, schema) = credential_schema::create_schema(did, &name, &version, &data)
        .await
        .map_err(HarnessError::from)?;
    credential_schema::publish_schema(did, wallet_handle, pool_handle, &schema)
        .await
        .map_err(HarnessError::from)?;
...
impl Agent {
    pub async fn create_schema(&mut self, schema: &Schema) -> HarnessResult<String> {
        ...
        let attrs = serde_json::to_string(&schema.attributes).map_err(HarnessError::from)?;
        let (schema_id, schema_json) = match create_and_publish_schema(
            &self.config.did,
        ...
        .await
        {
            Err(err) => {
                warn!(
                    "Error: {:?}, schema probably exists on ledger, skipping schema publish",
                    err
                );
                credential_schema::create_schema(
                    &self.config.did,
                    &schema.schema_name.to_string(),
                    &schema.schema_version.to_string(),
                    &attrs,
    ...
    pub fn get_schema(&mut self, id: &str) -> HarnessResult<String> {
        let schema: String = self.dbs.schema.get(id).ok_or(HarnessError::from_msg(
            HarnessErrorType::NotFoundError,
            &format!("Schema with id {} not found", id),
        ))?;
        let schema: serde_json::Value =
            serde_json::from_str(&schema).map_err(HarnessError::from)?;
        Ok(schema.to_string())
    }
}
