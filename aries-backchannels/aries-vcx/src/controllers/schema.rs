...
async fn create_and_publish_schema(
    ...
    let (schema_id, schema) = credential_schema::create_schema(did, &name, &version, &data).await.map_err(HarnessError::from)?;
    credential_schema::publish_schema(did, wallet_handle, pool_handle, &schema).await.map_err(HarnessError::from)?;
    ...
    let schema_json: serde_json::Value = serde_json::from_str(&schema_json).map_err(HarnessError::from)?;
...
impl Agent {
    ...
    pub fn get_schema(&mut self, id: &str) -> HarnessResult<String> {
        ...
        let schema: serde_json::Value = serde_json::from_str(&schema).map_err(HarnessError::from)?;
        Ok(schema.to_string())
    }
}
