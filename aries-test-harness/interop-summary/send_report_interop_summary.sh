...
print_help()
{
	...
	printf 'Usage: %s [-p|--test_project <arg>] [-h|--help] <allure_server>\n' "$0"
...
{
	local _required_args_string="'allure_server'"
...
assign_positional_args()
{
	...
	_positional_names="_arg_allure_server "
...
# Set the current Date for use in the file name of the results
DATE=`date +%m%d%y`
#d=`date +%y%m%d%H%M%S`
...
curl -X GET "$ALLURE_SERVER/allure-docker-service/report/export?project_id=$PROJECT_ID" -H  "accept: */*" --output $PROJECT_ID-allure-docker-service-report.zip

echo "------------------UNZIP-RESULTS------------------"
unzip $PROJECT_ID-allure-docker-service-report.zip
#unzip -d /dest/directory/ $PROJECT_ID-allure-docker-service-report.zip

echo "------------------PULL-RENAME-STORE-SUMMARY------------------"
mkdir ./allure-pulled-results
mv ./allure-report/widgets/summary.json ./allure-pulled-results/$PROJECT_ID-$DATE-summary.json
mv ./allure-report/widgets/environment.json ./allure-pulled-results/$PROJECT_ID-$DATE-environment.json

echo "------------------CLEANUP-ZIP-RESULTS-------------------"
rm $PROJECT_ID-allure-docker-service-report.zip
rm -rf ./allure-report
