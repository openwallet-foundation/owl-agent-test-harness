...
def setup_already_connected(context, requester_connection_info_json, requester, responder):
    ...
    (resp_status, resp_text) = agent_backchannel_GET(responder_url + "/agent/command/", "active-connection", id=requester_did)
    if resp_status == 200:
        resp_json = json.loads(resp_text)
        ...
        raise Exception(f"Problem retreiving responder's ({responder}) connection id for active connection. Requester's ({requester}) active connection info: {resp_text}")
