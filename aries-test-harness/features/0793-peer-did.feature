...
Feature: Qualified DID - Unqualified DID Transition
   ...
   Scenario Outline: Establish a connection with DID Exchange between two agents utilizing qualified did:peer DIDs with responder not set to send the same Peer DID method
      Given we have "2" agents
         | name | role      | start_parameters   |
         | Acme | requester | <start_parameters> |
