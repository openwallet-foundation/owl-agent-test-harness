    ...
    if credential_data != None:
        for schema in context.schema_dict:
            try:
                ...
                print(FileNotFoundError + ': features/data/cred_data_' +
                      schema.lower() + '.json')
...
def step_impl(context, verifier, request_for_proof, prover):
    ...
    if context.connectionless:
        (resp_status, resp_text) = agent_backchannel_POST(context.verifier_url + "/agent/command/",
                                                          "proof-v2", operation="create-send-connectionless-request", data=presentation_request)
    else:
        presentation_request["presentation_request"]["connection_id"] = context.connection_id_dict[verifier][prover]

        # send presentation request
        (resp_status, resp_text) = agent_backchannel_POST(context.verifier_url +
                                                          "/agent/command/", "proof-v2", operation="send-request", data=presentation_request)
...
def step_impl(context, prover, presentation):
    ...
    try:
        presentation_json_file = open(
            'features/data/' + presentation + '.json')
        ...
        if context.current_cred_format == "indy":
            ...
            for (input_descriptor_id, record_ids) in record_ids.items():
                new_record_ids[input_descriptor_id] = [
                    context.credential_id_dict[cred_type_name][-1] for cred_type_name in record_ids]
    ...
    (resp_status, resp_text) = agent_backchannel_POST(prover_url + "/agent/command/", "proof-v2",
                                                      operation="send-presentation", id=context.presentation_thread_id, data=presentation)
    assert resp_status == 200, f'resp_status {resp_status} is not 200; {resp_text}'
...
def step_impl(context, verifier):
    ...
    (resp_status, resp_text) = agent_backchannel_POST(verifier_url + "/agent/command/",
                                                      "proof-v2", operation="verify-presentation", id=context.presentation_thread_id)
    assert resp_status == 200, f'resp_status {resp_status} is not 200; {resp_text}'
    resp_json = json.loads(resp_text)
    ...
    if context.support_revocation:
        ...
        context.credential_verification_dict[context.presentation_thread_id] = strtobool(
            resp_json["verified"])
