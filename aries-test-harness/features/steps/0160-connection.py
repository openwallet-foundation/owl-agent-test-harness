...
def step_impl(context, inviter, invitee):
    ...
    (resp_status, resp_text) = agent_backchannel_GET(inviter_url + "/agent/command/", "connection")
    assert resp_status == 200

    (resp_status, resp_text) = agent_backchannel_GET(invitee_url + "/agent/command/", "connection")
    assert resp_status == 200
